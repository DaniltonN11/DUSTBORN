<!-- GDD completo extraído de GDD_Dustborn_v1.1.md -->
# Dustborn — GDD v1.1
...
Comportamentos-chave: emboscadas, ataques dirigidos ao Colossus (cômodos e motor), roubo de inventário (em áreas expostas), perseguição dinâmica.
...
- Status do Colossus: energia, combustível, integridade (por setor), temperatura interna, alertas de clima.
...
- Painel do Colossus (UI multi-usuário): lista de tarefas ativas (ignição, reparos), roles atribuídos, status por cômodo.

Alertas visuais e sonoros: tempestade, ataque ao Colossus, falha crítica no motor, falta de água.
...
- Modelo preferido: servidor dedicado (autoridade do servidor para estado crítico do Colossus). Host migration como fallback para sessões P2P.
- Sincronização: estado do Colossus e economia authoritative; posição/entrada de players com previsão cliente e correção por servidor.
...
- UI: HUD modular, inventário grid, painel do Colossus.
...
- Definir UI wireframes para o painel do Colossus e inventário.
